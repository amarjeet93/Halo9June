...
class WishlistAdapter(private val bookingFlowBean: SettingModel.DataBean.BookingFlowBean?,
                      private val screenFlowBean: SettingModel.DataBean.ScreenFlowBean?) : RecyclerView.Adapter<ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        //product
        holder.tvPrice.text = mContext?.getString(R.string.discountprice_tag, AppConstants.CURRENCY_SYMBOL, data.fixed_price?.toFloatOrNull())
        if (data.fixed_price != data.display_price) {
            ...
            holder.tvActualPrice.text = mContext?.getString(R.string.discountprice_tag, AppConstants.CURRENCY_SYMBOL, data.display_price?.toFloatOrNull())
        } else {
        ...
        if (data.avg_rating > 0) {
            holder.rbProdRating.rating = data?.avg_rating ?: 0.0f
            holder.tvRating.text = mContext?.getString(R.string.reviews_text, data?.avg_rating)
        }

        if (data.purchased_quantity ?: 0 >= data.quantity ?: 0 ||data.quantity == 0) {
