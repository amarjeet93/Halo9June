...
class SavedCardsViewModel(dataManager: DataManager) :BaseViewModel<SavedCardsNavigator>(dataManager){
    ...
    val allCardLiveData by lazy { SingleLiveEvent<SavedData>() }
    ...
    fun setDefaultCard(param: DefaultCardRequest) {
        setIsLoading(true)

        compositeDisposable.add(dataManager.setDefaultCard(param)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe({ this.defaultCardResponse(it) }, { this.handleError(it) })
        )
    }

    private fun defaultCardResponse(it: SuccessModel) {

        setIsLoading(false)

        if (it.statusCode == NetworkConstants.SUCCESS) {
            navigator.onSetDefaultCard()
        } else if (it.statusCode == NetworkConstants.AUTHFAILED) {
    ...
    private fun validateStripe(it: ApiResponse<SavedData>?) {
        ...
        when (it?.status) {
            NetworkConstants.SUCCESS -> {
                if(it.data?.data.isNullOrEmpty()==false)
                {
                    setCardCount( it.data?.data?.count()?:0)
                    allCardLiveData.value =  it.data
                }
            }
    ...
    private fun validateSquare(it: ApiResponse<List<SavedCardList>>?) {
        setIsLoading(false)
        setCardCount(0)
        ...
        when (it?.status) {
            NetworkConstants.SUCCESS -> {
                ...
                savedCardLiveData.value = it.data
            }
            NetworkConstants.AUTHFAILED -> {
                navigator.onSessionExpire()
            }
