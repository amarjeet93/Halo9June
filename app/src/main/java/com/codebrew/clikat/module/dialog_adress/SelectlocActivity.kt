...
*/
    ...
    override fun onClick(p0: View) {
        ...
        when (p0) {
            ...
            btn_save_adrs -> {
                if (ed_extra_adrs.text.trim().isNotEmpty()) {
                    val mapParam = MapInputParam(latlng?.latitude.toString(), latlng?.longitude.toString()
                            , tv_location_name.text.toString(), ed_extra_adrs.text.trim().toString(), mType, mAddressId)
                    ...
                    mBinding.root.onSnackbar(getString(R.string.please_complete_address))
                }
            }
        }
    }
