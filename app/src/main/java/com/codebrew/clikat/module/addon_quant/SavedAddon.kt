...
class SavedAddon : BottomSheetDialogFragment(), DialogListener {
    ...
    private fun addCart(cartInfo: CartInfo) {
        ...
        if (bookingFlowBean?.vendor_status == 0 && appUtils.checkVendorStatus(cartInfo.supplierId)) {
            mDialogsUtil.openAlertDialog(activity
                    ?: requireContext(), getString(R.string.clearCart,appUtils.loadAppConfig(0).strings?.supplier?:""), "Yes", "No", this)
        } else {
            if (appUtils.checkBookingFlow(requireContext(), cartInfo.productId, this)) {
                val quantity = appUtils.getCartList().cartInfos?.filter { it.productId == cartInfo.productId }?.sumBy { it.quantity }
                        ?: 0
                val remaingProd = cartInfo.prodQuant?.minus(cartInfo.purchasedQuant ?: 0) ?: 0
