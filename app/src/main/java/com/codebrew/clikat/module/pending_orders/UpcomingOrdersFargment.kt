 ...
 /*           if (viewModel.pendingLiveData.value != null && firstPage) {
   ...
   //         }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val screenFlowBean = prefHelper.getGsonValue(DataNames.SCREEN_FLOW, SettingModel.DataBean.ScreenFlowBean::class.java)
        appType = screenFlowBean?.app_type

        upcomingAdapter = UpcomingAdapter(activity, orderHistory2List, appType ?: 0,settingData, appUtils,orderUtils)
    ...
    fun apiCancelOrder() {
        if (isNetworkConnected) {
            viewModel.cancelOrder(upcomingAdapter?.selectedOrderId.toString())
        }
    }
