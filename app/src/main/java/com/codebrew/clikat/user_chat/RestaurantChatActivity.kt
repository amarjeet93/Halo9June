...
class RestaurantChatActivity : BaseActivity<ActivityUserChatBinding, UserChatViewModel>(),
    ...
    private var isRestaurantChat = false
    ...
    private val socketManager
            by lazy { SocketManager.getInstance(mDataManager.getKeyValue(PrefenceConstants.USER_ID, PrefenceConstants.TYPE_STRING).toString(),
                    mDataManager.getKeyValue(PrefenceConstants.DB_SECRET, PrefenceConstants.TYPE_STRING).toString(),supplierId) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent != null) {
            ...
            if (intent.hasExtra("isRestaurant")) {
                isRestaurantChat = intent.getBooleanExtra("isRestaurant",false)
            }
        }
    ...
    private fun callApi(isFirst: Boolean) {
        if (isNetworkConnected) {
            viewModel.fetchAllChat(orderId, isFirst, "2")
        }
    }
